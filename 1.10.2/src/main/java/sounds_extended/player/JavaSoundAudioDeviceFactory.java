...
public class JavaSoundAudioDeviceFactory extends AudioDeviceFactory
{
		...
		throws JavaLayerException
	{
		...
		try
		{
			ArrayList<String> ArrayLocation = new ArrayList<String>(Arrays.asList(getClass().getName().split("\\.")));
			ArrayLocation.remove(ArrayLocation.size() - 1);
			String ArrayString = "";
			for(String s : ArrayLocation)
				ArrayString += s + ".";
			ArrayString += "JavaSoundAudioDevice";
			JavaSoundAudioDevice dev = (JavaSoundAudioDevice)instantiate(loader, ArrayString);
			return dev;
		}
		catch (Exception ex)
		{
